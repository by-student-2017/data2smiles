...
# ==============================
# 6. Conditioned sampling (from target y)
# ==============================
...
cond_vec = preprocessor.transform(cond_row)  # [1, cat+num]
...
cond_t = torch.tensor(cond_final, dtype=torch.float32, device=device)

# Sample Generation
model.eval()
def sample_smiles(num_samples=200, temperature=1.2, top_k=None):
    samples = []
    with torch.no_grad():
        for _ in range(num_samples):
            z = torch.randn((1, ZDIM), device=device)
            logits = model.decode(z, cond_t, max_len=max_len-1, teacher_tokens=None)
            ...
            for t in range(probs.size(0)):
                ...
                if top_k is not None:
                    top_idx = torch.topk(p, top_k).indices
...
TOPK = 10
generated = sample_smiles(num_samples=400, top_k=TOPK)
if len(generated) == 0:
    print("Generation failed (all invalid). Relax the conditions or extend the learning.")
    sys.exit(0)
